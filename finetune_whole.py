"""
place data in ./downloaded_data folder
...
def test_whole_dataset(backbone_path):
    save_path['backbone'] = backbone_path
    if preprocessed:
        with open(file_path, 'rb') as f:
            data = pickle.load(f)
            X_train_all, y_train_all, X_test_all, y_test_all = data['X_train_all'], data['y_train_all'], data[
                'X_test_all'], \
                data['y_test_all']
    else:
        dataset_name = test_dataset_name
        dataset = globals()[dataset_name](base_folder)
        # pre-splited training and testing set
        X_train_all, y_train_all, X_test_all, y_test_all = dataset.get_train_test_seperated_shallow_net(
    ...
    result = []
    for subject_id, (X_train, y_train, X_test, y_test) in enumerate(
            zip(X_train_all, y_train_all, X_test_all, y_test_all)):  # for each subject in this dataset
        if model_type == 0:
        ...
        model = FineTuneNet(backbone, test_task_name_and_infos[test_dataset_name], size_after_pool).to(device)

        # different fine-tuning mode
        if mode == 0:
            for name, param in backbone.named_parameters():
                param.requires_grad = False
            optimizer = torch.optim.Adam([
                {'params': backbone.parameters(), 'lr': 0},
                {'params': model.test_head.parameters()}], lr=learning_rate, weight_decay=weight_decay)

        elif mode < 3:
            for name, param in backbone.named_parameters():
                if "_m" + str(mode) in name:
                    param.requires_grad = False

            optimizer = torch.optim.Adam([
                {'params': filter(lambda p: p.requires_grad, backbone.parameters()), 'lr': backbone_learning_rate},
                {'params': model.test_head.parameters()}], lr=learning_rate, weight_decay=weight_decay)
        ...
        X_train, X_dev, y_train, y_dev = sklearn.model_selection.train_test_split(X_train, y_train, test_size=0.2)
        ...
        train_data_loader = torch.utils.data.DataLoader(TensorDataset(X_train, y_train), batch_size=batch_size,
                                                        shuffle=True)
        dev_data_loader = torch.utils.data.DataLoader(TensorDataset(X_dev, y_dev), batch_size=128, shuffle=False)
        test_data_loader = torch.utils.data.DataLoader(TensorDataset(X_test, y_test), batch_size=128, shuffle=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--p_dataset_type', type=int, default=1)
    ...
    # basic info for pre-training and fine-tuning datasets, all path for models
    if dataset_type == 0:
        test_dataset_name = "BCIC4_2a"
    ...
    if model_type == 0:
        ...
        size_after_pool = 2560  # can be calculated automatically, need to optimize
    elif model_type == 1:
        save_path = {
    ...
    save_path['whole_model_data'] = f'preprocessed_data/{test_dataset_name}_whole_model_data.pkl'

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # device = torch.device('cpu')  # used when debugging

    file_path = save_path['whole_model_data']
    ...
    # used only to load pre-trained models weights easier
    ...
    result_matrix = np.zeros((len(backbone_path), num_subjects))

    if p_dataset_name == test_dataset_name:
        pass
    else:
        for i, path in enumerate(backbone_path):
            result = test_whole_dataset(path)
            result_matrix[i] = np.asarray(result)
        result_matrix = np.transpose(result_matrix)
        with open(f'result/{match_str}_net_whole_dataset_pre-trained_by_{p_dataset_name}_test_by{test_dataset_name}.pkl','wb') as f:
